...
var app = new Vue({
    ...
    computed: {
        filteredDoas() {
            ...
            if (this.searchQuery) {
                ...
                if (query === 'hadist') {
                    filtered = filtered.filter(d => d.source.toLowerCase() === 'hadist');
                } else if (query === 'quran') {
                    filtered = filtered.filter(d => d.source.toLowerCase() === 'quran');
                } else if (query === 'lainnya') {
                    filtered = filtered.filter(d => d.source.toLowerCase() === 'lainnya');
                }
            }
            return filtered;
        },
    ...
    methods: {
        ...
        arabicToLatin(arabicText) {
            // Peta transliterasi yang lebih akurat
            const arabicToLatinMap = {
                'ا': 'a', 'ب': 'b', 'ت': 't', 'ث': 'th', 'ج': 'j', 'ح': 'h', 'خ': 'kh',
                'د': 'd', 'ذ': 'dh', 'ر': 'r', 'ز': 'z', 'س': 's', 'ش': 'sh', 'ص': 's',
                'ض': 'd', 'ط': 't', 'ظ': 'z', 'ع': "'a", 'غ': 'gh', 'ف': 'f', 'ق': 'q',
                'ك': 'k', 'ل': 'l', 'م': 'm', 'ن': 'n', 'ه': 'h', 'و': 'w', 'ي': 'y',
                'ء': "'", 'آ': 'aa', 'إ': 'i', 'أ': 'a', 'ؤ': 'u', 'ئ': 'i',
                'َ': 'a', 'ِ': 'i', 'ُ': 'u', 'ً': 'an', 'ٍ': 'in', 'ٌ': 'un',
                'ْ': '', 'ّ': '' // Hapus sukun dan tasydid
            };
            ...
            // Pembersihan teks
            latin = latin.replace(/[\u064B-\u065F]/g, ''); // Hapus semua harakat
            latin = latin.trim(); // Hapus spasi di awal dan akhir
        
            // Format teks: kapitalisasi huruf pertama dan pisahkan kata
            latin = latin.split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
        ...
        fetchAllDoas() {
            ...
            for (let i = 1; i <= 108; i++) {
                ...
                promises.push(
                    fetch(url)
                        ...
                        .then(res => {
                            console.log(`Respons untuk ID ${i}:`, res);
            ...
            Promise.all(promises).then(results => {
                ...
                console.log('Doa yang dimuat:', this.allDoas);
                this.loading = false;
            }).catch(err => {
        ...
        getDetail(doaId) {
            ...
            fetch(url)
                .then(response => response.json())
                .then(res => {
                    console.log('Respons API untuk detail:', res);
                    if (res && res.status === true && res.data && res.data.arab && res.data.indo) {
                        this.detail = {
                            id: doaId,
                        ...
                        console.warn('Respons API tidak valid atau data tidak lengkap:', res);
                        this.detail = {
                            id: doaId,
                            judul: 'Data Tidak Ditemukan',
                            arab: '',
    ...
    mounted() {
        ...
        $('#detailModal').on('hidden.bs.modal', () => {
            ...
            this.loading2 = false;
            this.showShareOptions = false; // Reset opsi share saat modal ditutup
        });
    }
});
